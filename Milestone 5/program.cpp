...
ifstream in("../Milestone_Input/Milestone 5/Source.txt");
ifstream poi("../Milestone_Input/Milestone 5/POI.txt");
...
public:
    ...
    void solve()
    {
        ...
        for(int i = 0; i < polygons.size();i++){
            ...
            bool flag = true;
            for(int j = 0; j < polygons[i].lengths.size();j++){
                if(polygons[i].lengths[j] != match.lengths[j]){
                    flag = false;
                    break;
                }
            }
